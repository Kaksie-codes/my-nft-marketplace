...
//   // Holds custom category text when user selects "Other"
//   const [customCategory, setCustomCategory] = useState('');
//   const [mintStep, setMintStep] = useState<MintStep>('form');
...
//   // ── Fetch user's collection addresses from the factory ──
...
//   // ── Batch read name + symbol for every collection ──
//   // Wrapped in useMemo so the array reference stays stable between renders.
//   // Without useMemo, flatMap creates a new array on every render which
//   // causes useReadContracts to re-fire unnecessarily or not fire at all.
//   // functionName must be `as const` so TypeScript treats it as a literal
//   // type ("name" | "symbol") not a plain string — required by viem's types.
//   const collectionCalls = useMemo(() =>
//     collectionAddresses.flatMap((addr) => [
//       {
...
//   const { data: collectionMetadata } = useReadContracts({
//     contracts: collectionCalls,
//     query: { enabled: collectionAddresses.length > 0 },
//   });

//   // Helper: get name and symbol for the collection at a given index.
//   // Results are interleaved: index 0 → [name, symbol], index 1 → [name, symbol]
//   // So name  is at position: index * 2
//   //    symbol is at position: index * 2 + 1
//   const getCollectionInfo = (index: number): { name: string; symbol: string } => {
//     if (!collectionMetadata) return { name: 'Loading...', symbol: '...' };
//     const name   = (collectionMetadata[index * 2]?.result     as string) || 'Unnamed';
//     const symbol = (collectionMetadata[index * 2 + 1]?.result as string) || '???';
//     return { name, symbol };
//   };
...
//   // Handle onchain transaction failure
//   useEffect(() => {
//     if (isFailed) {
//       setErrorMessage('Transaction failed onchain. Check Etherscan for details.');
//       setMintStep('error');
...
//   const addProperty = () => {
//     setProperties([...properties, { id: Date.now().toString(), trait_type: '', value: '' }]);
//   };
...
//                     const { name: collectionName, symbol } = getCollectionInfo(index);
//                     const isSelected = formData.collection === addr;
//                     return (
//                       <button
//                         key={addr}
//                         type="button"
...
//                         {/* Name, symbol and shortened address */}
//                         <div className="min-w-0">
//                           {/* Primary: collection name + symbol */}
//                           <p className={`text-sm font-semibold truncate ${isSelected ? 'text-white' : 'text-main'}`}>
//                             {collectionName}
//                             <span className={`ml-1 text-xs font-normal ${isSelected ? 'text-white/70' : 'text-muted'}`}>
//                               ({symbol})
//                             </span>
//                           </p>
//                           {/* Secondary: shortened contract address */}
//                           <p className={`text-xs font-mono mt-0.5 ${isSelected ? 'text-white/60' : 'text-muted'}`}>
