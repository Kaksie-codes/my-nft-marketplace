...
const CreateNFTPage: React.FC = () => {
  ...
  // Holds the custom category text when the user selects "Other".
  // If they type something here it gets passed to mintNFT instead of "other".
  // If left blank, falls back to "other".
  ...
  // Parse NFTMinted event — field names match the updated contract
  useEffect(() => {
  ...
  // Handle onchain failure (transaction submitted but reverted)
  useEffect(() => {
    if (isFailed) {
  ...
  const handleInputChange = (field: keyof NFTFormData, value: string) => {
    // Clear custom category text when switching away from "other"
  ...
  const handleMint = async (e: React.FormEvent) => {
    ...
    // - "other" selected + user typed something → use their custom value (lowercased)
    // - "other" selected + nothing typed        → fall back to "other"
    // - Any other category selected             → use it directly
    const finalCategory =
