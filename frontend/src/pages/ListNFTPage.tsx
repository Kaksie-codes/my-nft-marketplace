...
// ── Contract auction duration constraints (mirrors Solidity constants) ────────
const MIN_AUCTION_SECS = 60 * 60;           // 1 hour
const MAX_AUCTION_SECS = 30 * 24 * 60 * 60; // 30 days

// ── Duration options — all stored in seconds ──────────────────────────────────
...
function formatEndDate(seconds: number): string {
...
const ListNFTPage = () => {
  ...
  // Form — duration stored in seconds, defaulting to 7 days
  const [listingType, setListingType] = useState<ListingType>('fixed');
  const [price,       setPrice]       = useState('');
  const [durationSecs, setDurationSecs] = useState(DURATIONS[5].seconds); // 7 days default
  ...
  const durRef      = useRef(durationSecs);
  const typeRef     = useRef(listingType);
  useEffect(() => { priceRef.current = price; },           [price]);
  useEffect(() => { durRef.current = durationSecs; },      [durationSecs]);
  ...
  useEffect(() => {
    ...
    // Duration is already in seconds — pass directly to contract
    const durSecs  = BigInt(durRef.current);
  ...
  const handleSubmit = () => {
    ...
    // Guard against duration going outside contract limits (shouldn't happen
    // with the preset buttons, but protects against future custom input)
    if (listingType === 'auction') {
      if (durationSecs < MIN_AUCTION_SECS) {
        setFormError('Auction duration must be at least 1 hour.');
        return;
      }
      if (durationSecs > MAX_AUCTION_SECS) {
        setFormError('Auction duration cannot exceed 30 days.');
        return;
      }
    }
  ...
  // Human-readable duration label for success screen
  const selectedDuration = DURATIONS.find(d => d.seconds === durationSecs);
  const durationLabel    = selectedDuration?.label ?? `${durationSecs}s`;
        ...
        {step === 'success' && (
          <div className="bg-surface border border-muted rounded-2xl p-8 text-center space-y-4">
            ...
            <p className="text-muted text-sm">
              ...
              {listingType === 'auction' ? ` — ${durationLabel} auction` : ' at fixed price'}.
            </p>
            {listTx && (
          ...
          <div className="space-y-6">
            ...
            {listingType === 'auction' && (
              <div>
                ...
                <p className="text-xs text-muted mb-3">Between 1 hour and 30 days.</p>
                <div className="flex flex-wrap gap-2">
                  {DURATIONS.map(d => (
                    <button key={d.seconds} onClick={() => setDurationSecs(d.seconds)}
                ...
                <p className="text-xs text-muted mt-2">
                  Ends: {formatEndDate(durationSecs)}
                </p>
              </div>
            )}
